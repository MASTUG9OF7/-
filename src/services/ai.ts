...
2. 两大核心叙事模型:
- 模型 A [觉醒复仇-价值重估]: 长期奉献的主角在觉醒事件后转为“利己”，实现自我价值，让旧家庭追悔莫及。
...
export async function generateStoryOutline(model: NarrativeModel, trigger: string, customApiKey?: string): Promise<StoryOutline> {
...
export async function* generateActSegmentStream(
  ...
  const stylePrompts: Record<LanguageStyle, string> = {
    colloquial: "语言风格口语化、接地气，符合60岁左右女性的口吻，节奏快，打脸狠。",
    elegant: "语言风格优雅知性，文字优美且富有哲理，展现主角从容淡定的气质。",
  ...
  当前部分：第 ${actNumber} 幕 - ${act.title} (第 ${segmentIndex + 1} / ${totalSegments} 段)
  本幕大纲：${act.outline}
  
  ${previousContext ? `前情提要：\n${previousContext}\n` : ""}
  
  写作要求：
  1. 请为本段创作约1000-1200字的详细内容。
  2. 使用第一人称（“我”）叙述，多用心理独白。
  3. ${stylePrompts[style]}
  4. 重点突出“价值补偿”和“权力反转”的爽感。
  5. 保持情节连贯，确保与前文衔接自然。
  6. 如果是该幕的最后一段，请做好向下一幕的过渡。`;
...
export async function generateTriggers(model: NarrativeModel, customApiKey?: string): Promise<string[]> {
  const ai = new GoogleGenAI({ apiKey: customApiKey || process.env.GEMINI_API_KEY || "" });
